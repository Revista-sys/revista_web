...
namespace App\Http\Controllers\Admin\PrimeMember;
...
//use App\Enums\ExportFileNames\Admin\PrimeMember as PrimeMemberExport;
...
use App\Services\PrimeMemberService;
...
class PrimeMemberController extends BaseController
{
    use PaginatorTrait;
    ...
    public function __construct(
        //private readonly CustomerRepositoryInterface        $customerRepo,
         ...
         private readonly PrimeMemberRepositoryInterface        $primememberRepo,
    ...
    {
        $primemembers = $this->primememberRepo->getListWhere(
...
foreach ($primemembers as $primemember) {
        $createdAt = Carbon::parse($primemember->created_at);

        if ($primemember->duration == 1) {
            $primemember->expiry_date = $createdAt->addMonth();
        } elseif ($primemember->duration == 2) {
            $primemember->expiry_date = $createdAt->addMonths(6);
        } elseif ($primemember->duration == 3) {
            $primemember->expiry_date = $createdAt->addYear();
        } else {
            $primemember->expiry_date = $createdAt; // Handle unknown duration
        }
    }
        return view(PrimeMember::LIST[VIEW], [
            'primemembers' => $primemembers,
        ]);
    }

    // public function updateStatus(Request $request): JsonResponse
    // {
    //     $this->primememberRepo->update(id:$request['id'],data:['is_active'=>$request->get('status', 0)]);
    //     $this->primememberRepo->deleteAuthAccessTokens(id:$request['id']);
    //     $customer = $this->primememberRepo->getFirstWhere(params: ['id'=>$request['id']]);
    ...
    // {
    //     $customer = $this->primememberRepo->getFirstWhere(params: ['id'=>$id]);
    //     if (isset($customer)) {
    ...
    //     $this->primememberRepo->delete(params:['id'=>$id]);
    ...
    // {
    //     $customers = $this->primememberRepo->getListWhere(
    //         searchValue: $request->get('searchValue'),
    //         filters: ['withCount'=>'orders'],
    //         relations: ['orders'],
    //         dataLimit: 'all'
    ...
    //             'customers' => $customers,
    //             'searchValue' => $request->get('searchValue'),
    //             'active' => $this->primememberRepo->getListWhere(filters:['is_active'=>1])->count(),
    //             'inactive' => $this->primememberRepo->getListWhere(filters:['is_active'=>0])->count(),
    ...
    // public function getPrimeMemberList(Request $request): JsonResponse
    // {
    //     $primemembers = $this->primememberRepo->getCustomerNameList(
    //         request:$request,
    //         dataLimit: getWebConfig(name:'pagination_limit')
    //     );
    //     return response()->json($primemembers);
    // }
    // public function add(CustomerRequest $request,CustomerService $customerService):RedirectResponse
    // {
    //     $this->primememberRepo->add($customerService->getCustomerData(request: $request));
