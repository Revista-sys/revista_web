...
class RegisterController extends Controller
{
    ...
    public function register(): View
    {
       ...
       $plandata = Plan::where(['status' => 1])->get();
        return view('web-views.customer-views.auth.register',['plandata'=>$plandata]);
    }


 public function prime($id): View
    {
$userdata = User::find($id);
    ...
    $plandata = Plan::where(['status' => 1])->get();
        ...
        return view('web-views.customer-views.auth.prime',['plandata'=>$plandata,'userdata'=>$userdata,'email'=>$userdata['email']]);
    }
        ...
        if ($request->ajax()) {
            ...
            return response()->json([
               ...
               // 'redirect_url' => theme_root_path() == 'default' ? route('customer.auth.login') : '',
                 ...
                 'redirect_url' => theme_root_path() == 'default' ? route('customer.auth.prime',[$user->id]) : '',
            ...
            return redirect(route('customer.auth.prime',[$user->id]));
...
public function subscribe(Request $request)
{
    try 
    {
        ...
        $checkoutemail = $request->input('email');
        ...
        $subscription = new PrimeSubscription();
        ...
        $response = $this->client->post('https://apitest.myfatoorah.com/v2/SendPayment', [
            'json' => [ 
                ...
                'CallbackUrl' => "https://revista.lmscloud.in/prime-verify-payment",
                'ErrorUrl' => "https://revista.lmscloud.in/prime-payment-error",
                'Language' => 'en',
        ...
        {
            if (isset($responseBody->Data->InvoiceURL) && !empty($responseBody->Data->InvoiceURL)) 
            {
                // Update subscription with transaction reference
                $subscriptiondata = PrimeSubscription::where('customer_id', $userid)->first();
                if ($subscriptiondata) {
                    $subscriptiondata->transaction_ref = $responseBody->Data->InvoiceId;
                    $subscriptiondata->save();
                }

                // Return redirect URL
                return response()->json([
                    'redirect' => $responseBody->Data->InvoiceURL
                ]);
            }
        } 
        else 
