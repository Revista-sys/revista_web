...
use App\Models\MagzineProductTag;
...
class MagzineDetailsController extends Controller
{
    use ProductTrait;
...
//$slug='womens-georgette-a-line-mini-dress-TfkPcP';
...
$vendor = Seller::find($product->user_id);;




            return view(VIEW_FILE_NAMES['magzine_details'], compact('vendor','product', 'countWishlist', 'countOrder', 'relatedProducts',
    ...
    public function getThemeAster(string $slug): View|RedirectResponse
    {
        ...
        if ($product != null) {
            ...
            $productIds = MagzineProduct::active()->where(['added_by' => $product['added_by']])
    ...
    public function getThemeFashion($slug): View|RedirectResponse
    {
        ...
        if ($product != null) {
            ...
            $deliveryInfo = self::getProductDeliveryCharge(Magzineproduct: $product, quantity: $product['minimum_order_qty']);
    ...
    public function theme_all_purpose($slug): View|RedirectResponse
    {
        ...
        echo "ddvdvvdg";die;
        $product = MagzineProduct::active()->with(['reviews', 'seller.shop'])->withCount('reviews')->where('slug', $slug)->first();
        if ($product != null) {

            $tags = MagzineProductTag::where('product_id', $product->id)->pluck('tag_id');
            ...
            $relatedProducts = MagzineProduct::active()->with(['reviews', 'flashDealProducts.flashDeal'])->withCount('reviews')->where('category_ids', $product->category_ids)->where('id', '!=', $product->id)->limit(12)->get();
            ...
            $more_product_from_seller = MagzineProduct::active()->withCount('reviews')->where('added_by', $product->added_by)->where('id', '!=', $product->id)->where('user_id', $product->user_id)->latest()->take(5)->get();
            $more_product_from_seller_count = MagzineProduct::active()->where('added_by', $product->added_by)->where('id', '!=', $product->id)->where('user_id', $product->user_id)->count();

            if ($product->added_by == 'seller') {
                $products_for_review = MagzineProduct::active()->where('added_by', $product->added_by)->where('user_id', $product->user_id)->withCount('reviews')->get();
            } else {
                $products_for_review = MagzineProduct::where('added_by', 'admin')->where('user_id', $product->user_id)->withCount('reviews')->get();
            }
            ...
            $product_ids = MagzineProduct::where(['added_by' => $product->added_by, 'user_id' => $product->user_id])->pluck('id');
            ...
            // top_rated products
            $products_top_rated = MagzineProduct::with(['rating', 'reviews'])->active()
                ->withCount(['reviews'])->orderBy('reviews_count', 'DESC')
                ->take(12)->get();

            $products_this_store_top_rated = MagzineProduct::with(['rating', 'reviews'])->active()
                ->where(['added_by' => $product->added_by, 'user_id' => $product->user_id])
                ->withCount(['reviews'])->orderBy('reviews_count', 'DESC')
                ->take(12)->get();

            $products_latest = MagzineProduct::active()->with(['reviews', 'rating'])->latest()->take(12)->get();
