...
class MagzineViewController extends Controller
{
    //for seller Shop
...
//             if (!$shop) {
//                 Toastr::error(translate('magzine_does_not_exist'));
//                 return back();
//             }else {
...
//         $product_ids = MagzineProduct::active()->when($id == 0, function ($query) {
...
//         $products = MagzineProduct::withCount('reviews')->whereIn('id', $product_ids)->paginate(12);
...
//             $category = MagzineCategory::with(['childes.childes'])->where('position', 0)->find($category_id);
//             if ($category != null) {
...
//         $products = MagzineProduct::active()
//             ->withCount('reviews')
...
//         return view(VIEW_FILE_NAMES['magzine_view_page'], compact('products', 'shop', 'categories','current_date','seller_vacation_start_date','seller_vacation_status',
...
public function default_theme($request) {
    ...
    if ($business_mode == 'single') {
        Toastr::error(translate('access_denied!!'));
        return back();
    }

    $id = $request->input('id', 0); // Get id from request, default to 0 if not provided
    ...
    $product_ids = MagzineProduct::active()
        ->when($id == 0, function ($query) {
...
// print_r($product_ids);die;
    ...
    $avg_rating = Review::active()
        ->where('status', 1)
        ->whereIn('product_id', $product_ids)
        ->avg('rating');

    $total_review = Review::active()
        ->where('status', 1)
        ->whereIn('product_id', $product_ids)
        ->count();

    if ($id == 0) {
        $total_order = Order::where('seller_is', 'admin')
            ->where('order_type', 'default_type')
            ->count();
    } else {
        $seller = Seller::find($id);
        if ($seller) {
            $total_order = $seller->orders()
                ->where('seller_is', 'seller')
                ->where('order_type', 'default_type')
                ->count();
        } else {
            $total_order = 0; // Handle case where seller is not found
        }
    }
    ...
    $products = MagzineProduct::active()
        ...
        ->when(!empty($request->sub_sub_category_id), function ($query) use ($request) {
         ...
         ->when(!empty($request->magzine_start_date) && !empty($request->magzine_end_date), function ($query) use ($request) {
        $query->whereBetween('magzine_start_date', [$request->magzine_start_date, $request->magzine_end_date]);
    })
     ->when(empty($request->magzine_start_date) && empty($request->magzine_end_date), function ($query) {
        $today = now()->toDateString(); // Get current date in YYYY-MM-DD format
        $query->where(function ($query) use ($today) {
            $query->whereDate('magzine_start_date', '<=', $today)
                  ->whereDate('magzine_end_date', '>=', $today);
        });
    })
    ...
    // Fetch shop details based on $id
    $shop = $id == 0 ? ['id' => 0, 'name' => Helpers::get_business_settings('company_name')] : Shop::where('seller_id', $id)->first();
    ...
    public function theme_aster($request, $id){
        ...
        $product_rating = MagzineProduct::active()->with('rating')
            ->withCount('reviews')
            ->when($id == 0, function ($query) {
                return $query->where(['added_by' => 'admin']);
            })
        ...
        if($id == 0){
            ...
            $products_for_review = MagzineProduct::active()->where('added_by', 'admin')->withCount('reviews')->count();
            $featured_products = MagzineProduct::active()->with([
                'seller.shop',
            ...
            $products_for_review = MagzineProduct::active()->where('added_by', 'seller')->where('user_id', $seller->id)->withCount('reviews')->count();
            $featured_products = MagzineProduct::active()->with([
                'seller.shop',
                'wishList'=>function($query){
                    return $query->where('customer_id', Auth::guard('customer')->user()->id ?? 0);
        ...
        $products = MagzineProduct::active()
            ...
            ->when($request['data_from'] == 'featured_deal', function($query){
                ...
                $featured_deal_product_ids = FlashDealMagzineProduct::where('flash_deal_id',$featured_deal_id)->pluck('product_id')->toArray();
    ...
    public function theme_fashion($request, $id){
        ...
        $colors_collection = MagzineProduct::active()->withCount('reviews')->whereIn('id', $product_ids)
            ->where('colors', '!=', '[]')
            ->pluck('colors')
            ->unique()
            ->toArray();
        ...
        if($id == 0){
            $total_order = Order::where('seller_is','admin')->where('order_type','default_type')->count();
            $products_for_review = MagzineProduct::active()->where('added_by', 'admin')->withCount('reviews')->count();
            $featured_products = MagzineProduct::active()->where(['added_by'=>'admin','featured'=>'1'])
            ...
            $total_order = $seller->orders->where('seller_is','seller')->where('order_type','default_type')->count();
            $products_for_review = MagzineProduct::active()->where('added_by', 'seller')->where('user_id', $seller->id)->withCount('reviews')->count();
            $featured_products = MagzineProduct::active()->where(['added_by'=>'seller','user_id'=>$seller->id,'featured'=>'1'])
                                    ->with(['wishList'=>function($query){
                                        return $query->where('customer_id', Auth::guard('customer')->user()->id ?? 0);
                                    }])->withCount('reviews')->get();
        }
        ...
        foreach ($category_info_for_fashion as $category_id) {
            $category = MagzineCategory::withCount(['product'=>function($qc1) use($id){
    ...
    public function ajax_filter_products(Request $request)
    {
        ...
        if($request->category)
        {
            ...
            {
                $cat_info = MagzineCategory::where('id', $category)->first();
                $index = array_search($cat_info->parent_id, $categories);
                if ($index !== false) {
                    array_splice($categories, $index, 1);
                }
            }
            $category = MagzineCategory::whereIn('id', $request->category)
                ->select('id', 'name')
                ->get();
        }
        ...
        // products search
        $products = MagzineProduct::active()->withCount('reviews')->withSum('orderDetails', 'qty', function ($query) {
                $query->where('delivery_status', 'delivered');
            })
            ->with(['wishList'=>function($query){
                return $query->where('customer_id', Auth::guard('customer')->user()->id ?? 0);
    ...
    public function theme_all_purpose($request, $id){
        ...
        $colors_collection = MagzineProduct::active()->whereIn('id', $product_ids)
            ->where('colors', '!=', '[]')
            ->pluck('colors')
            ->unique()
            ->toArray();
        ...
        if($id == 0){
            $total_order = Order::where('seller_is','admin')->where('order_type','default_type')->count();
            $products_for_review = MagzineProduct::active()->where('added_by', 'admin')->withCount('reviews')->count();
            $featured_products = MagzineProduct::active()->withCount('reviews')->where(['added_by'=>'admin','featured'=>'1'])->get();
        }else{
            $seller = Seller::find($id);
            $total_order = $seller->orders->where('seller_is','seller')->where('order_type','default_type')->count();
            $products_for_review = MagzineProduct::active()->where('added_by', 'seller')->where('user_id', $seller->id)->withCount('reviews')->count();
            $featured_products = MagzineProduct::active()->withCount('reviews')->where(['added_by'=>'seller','user_id'=>$seller->id,'featured'=>'1'])->get();
        }
        ...
        if($request['tab'] == 'all_product'){
            ...
            foreach ($category_info_for_fashion as $category_id) {
                $category = MagzineCategory::withCount(['product' => function ($query) use ($id) {
        ...
        if($request['tab']== 'store' || null){
            //top-rated
            $top_rated = MagzineProduct::active()
                                ->when($id == 0, function($query){
                                    $reviews = Review::active()->select('product_id', DB::raw('AVG(rating) as count'))
                                        ->groupBy('product_id')
                                        ->orderBy("count", 'desc')->get();
            ...
            //new arrival
            $new_arrival = MagzineProduct::active()
                        ->when($id == 0, function ($query) {
                            return $query->where('added_by', 'admin');
                        })
                        ->when($id != 0, function ($query) use($id){
    ...
    public function ajax_shop_vacation_check(Request $request){
        ...
        if($temporary_close  || ($vacation_status  && $current_date >= $vacation_start_date && $current_date <= $vacation_end_date)){
            ...
            $product_data = MagzineProduct::find($request->id);
