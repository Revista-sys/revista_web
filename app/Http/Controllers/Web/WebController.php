...
class WebController extends Controller
{
    ...
    public function __construct(
        ...
        $this->MyFatoorahPayment = new MyFatoorahPayment($this->mfConfig);
    }
            ...
            $response = $this->client->post('https://apitest.myfatoorah.com/v2/SendPayment', [
                'json' => [
                    ...
                    'CustomerAddress' => [
                        'Block' => $user['address'],
                        'Street' => $user['address'],
                        'HouseBuildingNo' => $user['address'],
                        'Address' => $user['address'],
                        'AddressInstructions' => $user['address']
                    ]
                ],
                'headers' => [
...

    {
        ...
        if ($physical_product) {
            ...
            echo $couponDiscount . ", " . $orderWiseShippingDiscount . ", " . $getShippingCostSavedForFreeDelivery;

            $user = Helpers::get_customer($request);
            $callbackURL = route('myfatoorah.callback');;
            $curlData = [
                'CustomerName' => $user->name ?? 'Guest',
                'CustomerReference' => $user->name ?? 'Guest',
                'NotificationOption' => 'LNK',
                'InvoiceValue' => $amount,
                // 'DisplayCurrencyIso' => 'KWD',
                'PaymentMethodId' => '6',
                "CallbackUrl" => $this->backURL,
                "ErrorUrl" =>   $this->backURL,
            ];

            $response = $this->MyFatoorahPayment->getInvoiceURL($curlData);
    ...
    public function checkout()
    {
        try {
            ...
            //Get Environment url
            $isTest = $this->mfConfig['test_mode'];
            $vcCode = $this->mfConfig['country_iso'];
