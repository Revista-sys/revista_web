...
public function initiateMyFatoorahPayment($amount, $currency, $description, $checkoutemail, $checkoutname, $checkoutphone)
{
...
// echo $checkoutphone;
//         dd($checkoutemail);
         // $checkoutemail = 'deepak@gmail.com'; // Default email
         // $checkoutname = 'deepak sir';        // Default name
         // $checkoutphone = '1234567890';       // Default phone
        ...
        $response = $this->client->post('https://apitest.myfatoorah.com/v2/SendPayment', [
            'json' => [
                ...
                'CallbackUrl' => "https://revista.lmscloud.in/verify-payment",
                'ErrorUrl' => "https://revista.lmscloud.in/payment-error",
                'Language' => 'en',
...
//dd($request);
  $user = Helpers::get_customer($request);
            $response = $this->initiateMyFatoorahPayment($amount, 'KWD', 'Payment', $user['email'], $user['name'], $user['phone']);
            ...
            //   }
 if (isset($response->Data->InvoiceURL) && !empty($response->Data->InvoiceURL)) {

// print_r($order_ids);
// print_r($response->Data->InvoiceId);die;

// foreach($order_ids  as $oid)
// {
//  $order = Order::where('id', $oid)->first();
//     if ($order) {
//         $order->transaction_ref = $response->Data->InvoiceId;
//         $order->save();
//     }
...
foreach ($order_ids as $oid) {
    ...
    if ($order) {
        ...
        $order->save();
                ...
                return back()->with('error', 'Unable to retrieve payment URL.');
            }
...
public function checkout_complete_cod(Request $request)
    {
        // if($request->payment_method != 'cash_on_delivery'){
        //     return back()->with('error', 'Something went wrong!');
        // }
