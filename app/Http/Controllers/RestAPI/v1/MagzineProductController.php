...
$offset = $request->offset; // Example: Offset of 0
        ...
        $categories = MagzineCategory::with(['childes.childes', 'childes' => function ($query) {
...
public function list(Request $request)
{
    ...
    $categoryId = $request->category_id; // Get category_id from the request

    $query = MagzineProduct::withCount('reviews')
        ->where(['added_by' => 'seller'])
        ->orderBy('id', 'DESC')
        ->skip($offset)
        ->take($limit);
    ...
    public function product_filter(Request $request)
    {
        ...
        // products search
        $products = MagzineProduct::active()->with(['rating','tags'])
    ...
    public function get_product(Request $request, $slug)
    {
        ...
        $product = MagzineProduct::with(['reviews.customer', 'seller.shop','tags'])
    ...
    {
        $categories = MagzineCategory::where('home_status', true)->get();
    ...
    {
        try {
            $product = MagzineProduct::find($id);
            $overallRating = getOverallRating($product->reviews);
    ...
    {
        $product = MagzineProduct::where('slug', $product_slug)->first();
...
echo "hello";die;
 ...
 //        return response()->json($product, 200);
    ...
    public function get_shop_again_product(Request $request)
    {
        $user = Helpers::get_customer($request);
        if($user != 'offline') {
            $products = MagzineProduct::active()->with('seller.shop','reviews')
                ->withCount(['wishList' => function($query) use($user){
                    $query->where('customer_id', $user != 'offline' ? $user->id : '0');
                }])
