...
class PrimeRepository implements PrimeRepositoryInterface
{
    public function __construct(
        private readonly Plan $prime,
    )
    {
    }
    ...
    {
        return $this->prime->where($params)->with($relations)->first();
    }

    public function getList(array $orderBy = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->prime->with($relations)
    ...
    public function getListWhereIn(array $orderBy = [], string $searchValue = null, array $filters = [], array $whereInFilters = [], array $relations = [], array $nullFields = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->prime
            ...
            ->when($filters['theme'], function ($query) use($filters){
                //return $query->where('name', $filters['theme']);
            });
    ...
    {
        $this->prime->where('id', $id)->update($data);
        return true;
    }

    public function delete(array $params): bool
    {
        $this->prime->where($params)->delete();
        return true;
    }
}
