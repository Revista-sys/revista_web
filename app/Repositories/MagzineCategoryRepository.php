...
class MagzineCategoryRepository implements MagzineCategoryRepositoryInterface
{
    public function __construct(
        private readonly MagzineCategory       $category,
    ...
    public function getListWhere(array $orderBy = [], string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->category->with($relations)
            ...
            ->when(isset($searchValue), function ($query) use ($searchValue) {
                $translation_ids = $this->translation->where('translationable_type', 'App\Models\MagzineCategory')
                    ->where('key', 'name')
                ...
                foreach ($category->childes as $child) {
                    if ($child->childes) {
                        foreach ($child->childes as $item) {
                            $this->translation->where('translationable_type', 'App\Models\MagzineCategory')->where('translationable_id', $item['id'])->delete();
                            $this->category->where('id', $item['id'])->delete();
                        }
                    }
                    $this->translation->where('translationable_type', 'App\Models\MagzineCategory')->where('translationable_id', $child['id'])->delete();
                    $this->category->where('id', $child['id'])->delete();
                }
            }
        }

        $this->translation->where('translationable_type', 'App\Models\MagzineCategory')->where('translationable_id', $params['id'])->delete();
