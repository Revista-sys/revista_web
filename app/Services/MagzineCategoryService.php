...
class MagzineCategoryService
{
    ...
    public function getAddData(object $request): array
    {
        return [
            ...
            'icon' => $this->upload('category/', 'webp', $request->file('image12')),
    ...
    {
        $image = $request->file('image12') ? $this->update('category/', $data['image'], 'webp', $request->file('image12')) : $data['icon'];
        return [
            'name' => $request['name'][array_search('en', $request['lang'])],
            'slug' => Str::slug($request['name'][array_search('en', $request['lang'])]),
            'icon' => $image,
