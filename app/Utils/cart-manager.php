...
class CartManager
{
  ...
  $product = \App\Models\Product::where('id', $item['product_id'])->first();
    ...
    if ($product) {
        if ($authuser == 1 && $product->prime_product_status == 0) {
            // Product is a prime product and user is a prime member
            $product_subtotal = ($item['price'] - $item['discount']) * $item['quantity'];
        } else {
            // Product is not a prime product or user is not a prime member
            $product_subtotal = $item['price'] * $item['quantity'];
        }
    } else {
        // Product not found, use the regular price
        $product_subtotal = $item['price'] * $item['quantity'];
    }
