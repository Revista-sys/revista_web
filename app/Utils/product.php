...
// print_r($authuser);
  ...
  // if ($product->discount > 0 ) {
            ...
            return '<span class="ajay discounted_unit_price fs-24 font-bold ys">' . $productDiscountedPrice . '</span>' . '<del class="total_unit_price align-middle text-muted fs-18 font-semibold">' . webCurrencyConverter(amount: $productUnitPrice) . '</del>';
        }


// if ($authuser == 0 && $product->discount > 0 && $product->prime_product_status == 0) {
     


//   // if ($product->discount > 0 ) {
      
//           return '<span class="vijay2 total_unit_price fs-24 font-bold ys2">' . webCurrencyConverter(amount: $productUnitPrice) . '</span>';
//         }


         else {
            return '<span class="vijay total_unit_price fs-24 font-bold ys2">' . webCurrencyConverter(amount: $productUnitPrice) . '</span>';
        }
    }
}
...
if (!function_exists('getMagzineVendorProductsCount')) {
    function getMagzineVendorProductsCount(string $type):int
    {
        $products = MagzineProduct::where(['added_by'=>'seller'])->get();
        return match ($type) {
            'new-product' => $products->where('request_status', 0)->count(),
            'product-updated-request' => $products->whereNotNull('is_shipping_cost_updated')->where('is_shipping_cost_updated', 0)->count(),
            'approved' => $products->where('request_status', 1)->count(),
...
if (!function_exists('getHandmadeVendorProductsCount')) {
    function getHandmadeVendorProductsCount(string $type):int
    {
        $products = Product::where(['added_by'=>'seller','seller_type'=>2])->get();
        return match ($type) {
            'new-product' => $products->where('request_status', 0)->count(),
            'product-updated-request' => $products->whereNotNull('is_shipping_cost_updated')->where('is_shipping_cost_updated', 0)->count(),
            'approved' => $products->where('request_status', 1)->count(),
...
if (!function_exists('getAdminMagzineProductsCount')) {
    function getAdminMagzineProductsCount(string $type):int
    {
        $products = MagzineProduct::where(['added_by'=>'admin'])->get();
        return match ($type) {
            'all' => $products->count(),
            'new-product' => $products->where('request_status', 0)->count(),
            'product-updated-request' => $products->whereNotNull('is_shipping_cost_updated')->where('is_shipping_cost_updated', 0)->count(),
