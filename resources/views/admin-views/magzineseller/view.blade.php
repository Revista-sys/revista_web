...
@section('content')
                ...
                <ul class="nav nav-tabs flex-wrap page-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active"
                           href="{{ route('admin.magzinesellers.view',$seller['id']) }}">{{translate('shop_overview')}}</a>
                    </li>
                    @if ($seller['status']!="pending")
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.magzinesellers.view',['id'=>$seller['id'], 'tab'=>'order']) }}">{{translate('order')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.magzinesellers.view',['id'=>$seller['id'], 'tab'=>'product']) }}">{{translate('product')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.magzinesellers.view',['id'=>$seller['id'], 'tab'=>'setting']) }}">{{translate('setting')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.magzinesellers.view',['id'=>$seller['id'], 'tab'=>'transaction']) }}">{{translate('transaction')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.magzinesellers.view',['id'=>$seller['id'], 'tab'=>'review']) }}">{{translate('review')}}</a>
                        </li>
                    @endif
                </ul>
            </div>
                ...
                <div class="d-flex flex-wrap gap-3 justify-content-between">
                        ...
                        <div class="media-body">
                            ...
                            <div class="d-block">
                                ...
                                <div class="d-flex gap-3 flex-wrap mb-3 lh-1">
                                    ...
                                    <span class="border-left"></span>
                                    <a href="{{ $seller['status']!="pending" ? route('admin.magzinesellers.view',['id'=>$seller['id'], 'tab'=>'review']): 'javascript:' }}"
                    ...
                    @if ($seller['status']=="pending")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.magzinesellers.updateStatus')}}" id="reject-form" method="POST">
                                @csrf
                            ...
                            <form class="d-inline-block" action="{{route('admin.magzinesellers.updateStatus')}}" id="approve-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                    ...
                    @if ($seller['status']=="approved")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.magzinesellers.updateStatus')}}" id="suspend-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                    ...
                    @if ($seller['status']=="suspended" || $seller['status']=="rejected")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.magzinesellers.updateStatus')}}" id="active-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                                <input type="hidden" name="status" value="approved">
 ...
 @if ($seller['status']=='approved' && $seller['doc_image_approval_status']==0 && $seller['seller_type']==3)
                      <!--   <div style="display:none" class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                           
                            <form class="d-inline-block" action="{{route('admin.magzinesellers.updateDocStatus')}}" id="approve-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
...
<div class="col-xxl-6" style="display: none;">
                                <div class="bg-light p-3 border border-primary-light rounded">
                                    ...
                                    <div class="d-flex gap-5">
                                        <div class="pair-list">
                                            <div>
                                                <span class="key text-nowrap">{{'DOC Image'}}</span>
                                                <span class="px-2">:</span>
                                                


 <img width="50" style="cursor: pointer;" 
                                            class="avatar docimg " src="{{ getValidImage(path: 'storage/app/public/seller/'.($seller->doc_image ? $seller->doc_image:''), type: 'backend-basic') }}"
                                                alt="">
                                            </div>

                                            <div>
                                                <span class="key text-nowrap">{{'Doc Image2'}}</span>
                                                <span class="px-2">:</span>
                                               <img width="50" style="cursor: pointer;"
                                            class="avatar docimg " src="{{ getValidImage(path: 'storage/app/public/seller/'.($seller->doc_image1 ? $seller->doc_image1:''), type: 'backend-basic') }}"
                                                alt="">
