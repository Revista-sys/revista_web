                ...
                <ul class="nav nav-tabs flex-wrap page-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active"
                           href="{{ route('admin.handmadesellers.view',$seller['id']) }}">{{translate('shop_overview')}}</a>
                    </li>
                    @if ($seller['status']!="pending")
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.handmadesellers.view',['id'=>$seller['id'], 'tab'=>'order']) }}">{{translate('order')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.handmadesellers.view',['id'=>$seller['id'], 'tab'=>'product']) }}">{{translate('product')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.handmadesellers.view',['id'=>$seller['id'], 'tab'=>'setting']) }}">{{translate('setting')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.handmadesellers.view',['id'=>$seller['id'], 'tab'=>'transaction']) }}">{{translate('transaction')}}</a>
                        </li>
                        <li class="nav-item" style="display:none">
                            <a class="nav-link"
                               href="{{ route('admin.handmadesellers.view',['id'=>$seller['id'], 'tab'=>'review']) }}">{{translate('review')}}</a>
                        </li>
                    @endif
                </ul>
            </div>
...
<img class="avatar avatar-170 rounded-0"
     src="{{ $seller && $seller->shop && $seller->shop->image
                ? getValidImage(path: 'storage/app/public/shop/' . $seller->shop->image, type: 'backend-basic')
...
@if($seller->shop->temporary_close??'' || ($seller->shop->vacation_status??'' && $current_date >= date('Y-m-d', strtotime($seller?->shop->vacation_start_date ?? '')) && $current_date <= date('Y-m-d', strtotime($shop->vacation_end_date ?? ''))))
...
@endif
                            ...
                            <div class="d-block">
                                ...
                                <div class="d-flex gap-3 flex-wrap mb-3 lh-1">
                                    ...
                                    <span class="border-left"></span>
                                    <a href="{{ $seller['status']!="pending" ? route('admin.handmadesellers.view',['id'=>$seller['id'], 'tab'=>'review']): 'javascript:' }}"
  ...
  <div class="d-flex justify-content-sm-end flex-wrap gap-3 mb-3">
<form class="d-inline-block" action="{{route('admin.handmadesellers.updateCommissionStatus')}}" id="approve-form" method="POST">
    @csrf
    ...
    <div class="col-sm-9">
 Commisssion(%)
      <input type="text" class="form-control" name="sales_commission_percentage" id="sales_commission_percentage"
                value="{{$seller['sales_commission_percentage']}}"
                placeholder="{{''}}" >
    </div>
    <button type="button" class="btn btn-success px-5 form-alert"   data-id="approve-form">Submit</button>
</form>
</div>

                    @if ($seller['status']=="pending")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.handmadesellers.updateStatus')}}" id="reject-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                            ...
                            <form class="d-inline-block" action="{{route('admin.handmadesellers.updateStatus')}}" id="approve-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                    ...
                    @if ($seller['status']=="approved")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.handmadesellers.updateStatus')}}" id="suspend-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                    ...
                    @if ($seller['status']=="suspended" || $seller['status']=="rejected")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.handmadesellers.updateStatus')}}" id="active-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                                <input type="hidden" name="status" value="approved">
 ...
 @if ($seller['status']=='approved' && $seller['doc_image_approval_status']==0 && $seller['seller_type']==2)
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">

                            <form class="d-inline-block" action="{{route('admin.handmadesellers.updateDocStatus')}}" id="approve-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                                <input type="hidden" name="doc_status" value="1">
                                <button type="button" class="btn btn-success px-5 form-alert" data-message="{{'want_to_approve_this_vendor'.'?'}}" data-id="approve-form">{{'Approve Doc'}}</button>
...
<div class="col-xxl-6">
                                <div class="bg-light p-3 border border-primary-light rounded">
                                    <h4 class="mb-3 text-capitalize">{{'Doc Preview'}}</h4>

                                    <div class="d-flex gap-5">
                                        <div class="pair-list">
                                            <!-- <div>
                                                <span class="key text-nowrap">{{'IBAN DOC'}}</span>
                                                <span class="px-2">:</span>


                                                 <img width="50"
                                            class="avatar" src="{{ getValidImage(path: 'storage/app/public/seller/'.($seller->iban ? $seller->iban.'webp':''), type: 'backend-basic') }}"
                                                alt="">
                                            </div> -->

                                            <div>
                                                <span class="key text-nowrap">{{'Vat Doc'}}</span>
                                                <span class="px-2">:</span>
                                                <img width="50" style="cursor: pointer;"
                                            class="avatar docimg" src="{{ getValidImage(path: 'storage/app/public/seller/'.($seller->vat ? $seller->vat.'webp':''), type: 'backend-basic') }}"
                                                alt="">
                                            </div>
                                               <div>
                                                <span class="key text-nowrap">{{'Fda Doc'}}</span>
                                                <span class="px-2">:</span>
                                                  <img width="50" style="cursor: pointer;"
                                            class="avatar docimg" src="{{ getValidImage(path: 'storage/app/public/seller/'.($seller->fda ? $seller->fda.'webp':''), type: 'backend-basic') }}"
                                                alt="">
                                            </div>
                                        ...
                                        <div class="pair-list">
                                             ...
                                             <div>
                                                <span class="key text-nowrap">{{'Freelancer Doc'}}</span>
                                                <span class="px-2">:</span>
                                                  <img width="50" style="cursor: pointer;"
                                            class="avatar docimg" src="{{ getValidImage(path: 'storage/app/public/seller/'.($seller->freelancer ? $seller->freelancer.'webp':''), type: 'backend-basic') }}"
                                                alt="">
                                            </div>
...
<div class="d-flex gap-5 pt-3">
 <div class="pair-list">
                                            ...
                                            <div>
                                                <span class="key text-nowrap" style="min-width: 150px;">{{'National Address'}}</span>
                                                <span class="px-2">:</span>
                                                <span class="value"> {{$seller->national_address ?? translate('no_data_found') }}</span>
                                            </div>
                                        </div>
                                        </div>
...
<script>
 document.addEventListener('DOMContentLoaded', function() {
    const docImages = document.querySelectorAll('.docimg');
